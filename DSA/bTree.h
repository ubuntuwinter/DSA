...
protected:
	...
	void solveUnderflow(BTNodePosi(T) v); // 因插入而下溢之后的合并处理
public:
...
template<typename T>
inline void BTree<T>::solveUnderflow(BTNodePosi(T) v) { // 关键码删除后若节点下溢，则做节点旋转或合并处理
	if ((_order + 1) >> 1 <= v->child.size()) return; // 递归基：当前节点并未发生下溢
	...
	if (0 < r) { // 若v不是p的第一个孩子，则
		...
		if ((_order + 1) >> 1 < ls->child.size()) { // 若该兄弟足够“胖”，则
	...
	if (p->child.size() - 1 > r) { // 若v不是p的最后一个孩子，则
		...
		if ((_order + 1) >> 1 < rs->child.size()) { // 若该兄弟足够“胖”，则
	...
	// 情况3：左、右兄弟要么为空（但不可能同时），要么都太“廋”——合并
	if (0 < r) { // 与左兄弟合并
		BTNodePosi(T) ls = p->child[r - 1]; // 左兄弟必然存在
		ls->key.insert(ls->key.size(), p->key.remove(r - 1)); p->child.remove(r); // p的第r - 1个关键码转入ls，v不再是p的第r个孩子
		ls->child.insert(ls->child.size(), v->child.remove(0));
		if (ls->child[ls->child.size() - 1]) { // v最左侧的孩子过继给ls做右侧孩子
		...
		BTNodePosi(T) rs = p->child[r + 1]; // 右兄弟必然存在
		rs->key.insert(0, p->key.remove(r)); p->child.remove(r); // p的第r个关键码转入rs，v不再是p的第r个孩子
...
bool BTree<T>::remove(const T& e) { // 从BTree树中删除关键码e
	...
	if (v->child[0]) { // 若v非叶节点，则e的后继必属于某叶节点
		BTNodePosi(T) u = v->child[r + 1]; // 在右子树一直向左，即可
